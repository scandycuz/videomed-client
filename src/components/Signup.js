...
export class Signup extends Component {
  ...
  handleSubmit = async (e) => {
    ...
    if (!Object.values(this.state.errors).find((v) => v.length)) {
      ...
      if (!this.props.error) this.setState({ success: true });
    }
  }
      ...
      <Container width="36rem">
        <Box width="100%" marginTop="6rem" align="center">
          { this.state.success ? (
            <Container width="xs">
              <Box>
                <Typography size="1.25rem" align="center">
                  You&apos;re account has been succesfully created!
                  Please <Link to="/login" color="secondary.main"><strong>Log in</strong></Link>
                </Typography>
              </Box>
            </Container>
          ) : (
